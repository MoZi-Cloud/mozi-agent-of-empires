...
fn build_frontend() {
    ...
    // set, the directory is copied to web/dist/ and pnpm is not invoked.
    //
    ...
    // var is read by the pnpm child process below; we only need to tell Cargo
    ...
    assert!(
        ...
        "pnpm is required to build with --features serve. Install it through mise."
    );
    ...
    if !status.success() {
        panic!("pnpm run build failed in web/. Run `cd web && pnpm run build` to debug.");
    }
}
...
fn maybe_install_web_deps() {
    ...
    let node_modules_marker = Path::new("web/node_modules/.pnpm/lock.yaml");
    let package_json = Path::new("web/package.json");
    let package_lock = Path::new("web/pnpm-lock.yaml");
    ...
    // Prefer a frozen lockfile when one exists: it is deterministic and cleans
    // up drift from manual edits. Fall back to a regular install for projects
    ...
    println!(
        "cargo:warning=Installing web dependencies via `pnpm {install_cmd}` (node_modules is stale or missing)..."
    );
    ...
    let status = command
        .current_dir("web")
        .status()
        .unwrap_or_else(|e| panic!("Failed to spawn `pnpm {install_cmd}` in web/: {e}"));

    if !status.success() {
        panic!(
            "`pnpm {install_cmd}` failed in web/. \
             Run `cd web && pnpm {install_cmd}` to see the full error."
        );
    }
}
